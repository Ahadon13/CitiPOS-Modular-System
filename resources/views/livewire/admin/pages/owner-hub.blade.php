<div class="min-h-screen
            ...
            flex flex-col pt-8 sm:pt-16 pb-10 sm:pb-12 px-3 sm:px-6">
    <div class="max-w-4xl w-full mx-auto">
        ...
        <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-6 sm:mb-8">
            <div class="flex gap-3 sm:gap-4 items-center min-w-0">
                ...
                <div class="min-w-0">
                    <h1 class="text-xl sm:text-2xl font-bold text-neutral-900 dark:text-white tracking-tight break-words">
                        Welcome back, {{ auth()->user()->name ?? 'User' }}!
                    </h1>
                </div>
            </div>

            <x-ui.button variant="ghost" wire:click="logout" icon="arrow-right-start-on-rectangle" class="w-full sm:w-auto justify-center hover:text-blue-600 hover:bg-blue-50">
                Sign Out
            </x-ui.button>
        </div>
        ...
        <a href="{{ route('admin.dashboard') }}" class="block mb-10 group">
            <x-ui.card size="full" hoverless class="bg-white/60 hover:bg-white/90 transition-all duration-200 border-blue-100 shadow-[0_4px_20px_-4px_rgba(225,29,72,0.05)] backdrop-blur-sm">
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    ...
                    <x-ui.icon name="chevron-right" class="hidden sm:block size-5 text-neutral-300 group-hover:text-blue-500 transition-colors" />
                </div>
            </x-ui.card>
        </a>
            ...
            <x-ui.card size="full" hoverless class="bg-white/60 hover:bg-white/90 transition-all duration-200 border-blue-100 shadow-[0_4px_20px_-4px_rgba(225,29,72,0.05)] backdrop-blur-sm !p-4">
                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    ...
                    <div class="flex items-center gap-4 min-w-0">
                        ...
                        <div class="min-w-0">
                            <div class="flex items-center gap-2">
                                <h3 class="font-bold text-neutral-900 dark:text-white text-lg break-words">{{ $branch->name }}</h3>
                            </div>
                    ...
                    <div class="flex flex-col sm:flex-row sm:items-center justify-between sm:justify-end gap-4 sm:gap-6 border-t sm:border-t-0 border-blue-100/50 pt-3 sm:pt-0">
                        ...
                        <x-ui.button href="{{ route('admin.branches.view', $branch->id) }}" iconAfter="chevron-right" wire:loading.attr="disabled" class="w-full sm:w-auto justify-center bg-blue-500 hover:bg-blue-600 text-white border-0 shadow-md shadow-blue-500/20 px-5">
                            Enter
                        </x-ui.button>
                    </div>
