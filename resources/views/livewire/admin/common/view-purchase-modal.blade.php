<x-ui.modal id="view-purchase" width="5xl" heading="View Purchase Order" :closeByEscaping="true" :closeByClickingAway="true">
    <div class="space-y-6"
        x-data="{
            ...
            productName(item) {
                return item.product?.brand_name || item.product?.name || item.product?.product_code || 'Unknown product';
            },

            productDetail(item) {
                const product = item.product || {};
                const pharmacyDetail = [product.generic_name, product.dosage, product.form].filter(Boolean).join(' - ');
                const productCode = product.product_code ? `Code: ${product.product_code}` : null;

                return pharmacyDetail || productCode || 'No product detail';
            }
        }"
        x-init="
                ...
                <tbody class="divide-y divide-black/10 dark:divide-white/10 bg-white dark:bg-[#060A23]">
                    <template x-for="item in items" :key="item.id">
                        <tr class="hover:bg-neutral-50 dark:hover:bg-white/5 transition-colors">
                            <td class="px-4 py-3">
                                <p class="font-bold text-neutral-900 dark:text-white" x-text="productName(item)"></p>
                                <p class="text-xs text-neutral-500" x-text="productDetail(item)"></p>
                            </td>
                            <td class="px-4 py-3 text-center font-medium text-neutral-700 dark:text-neutral-300" x-text="item.unit?.name || '-'"></td>
                            <td class="px-4 py-3 text-center text-neutral-900 dark:text-white" x-text="parseFloat(item.quantity_ordered || 0)"></td>
                            <td class="px-4 py-3 text-center font-bold"
                                :class="parseFloat(item.quantity_received || 0) !== parseFloat(item.quantity_ordered || 0) ? 'text-orange-500' : 'text-neutral-900 dark:text-white'"
                                x-text="parseFloat(item.quantity_received || 0)">
                            </td>
                            ...
                            <td class="px-4 py-3 text-right font-bold text-neutral-900 dark:text-white"
                                x-text="formatMoney((parseFloat(item.quantity_received || 0) > 0 ? parseFloat(item.quantity_received || 0) : parseFloat(item.quantity_ordered || 0)) * (item.cost_per_unit?.amount || 0))">
                            </td>
                        </tr>
                    </template>
                </tbody>
