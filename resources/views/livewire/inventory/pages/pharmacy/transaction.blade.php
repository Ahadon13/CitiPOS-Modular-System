<div class="max-w-7xl mx-auto space-y-6" x-data="transactionManager()" data-receipt-route-template="{{ route('pos.pharmacy.sales.receipt', ['sale' => '__SALE_ID__']) }}">
                    ...
                    <x-ui.card hoverless size="full" class="relative overflow-hidden p-5 flex flex-col justify-between h-full group transition-all border border-black/5 dark:border-white/5 hover:border-blue-500/50 dark:hover:border-blue-400/50 hover:shadow-md">
                        ...
                        <div>
                            ...
                            <h3 class="font-bold text-neutral-900 dark:text-white line-clamp-2 leading-tight mb-1" title="{{ $item->product->brand_name }}">
                                {{ $item->product->brand_name }} - {{ $item->product->dosage ?? 'N/A' }}
                            </h3>
    ...
    {{-- Partnership Sales --}}
    @if($this->showPartnershipPanel)
        <x-ui.card hoverless size="full" class="p-0 border-blue-500/30">
                ...
                <div class="flex flex-col md:flex-row md:items-start justify-between gap-4">
                    <div>
                        ...
                        <p class="text-sm text-neutral-500 mt-1">
                            Sales charged at a partner price (LGU, DSWD and similar), shown against the regular price they replaced.
                        </p>
                    </div>
                            ...
                            <tr>
                                <td colspan="9" class="px-6 py-12 text-center text-neutral-500">
                                    <x-ui.empty>
                                        ...
                                        <x-ui.empty.contents>
                                            <x-ui.heading>No partnership sales</x-ui.heading>
                                            <x-ui.text>No partner-priced sales for the selected period.</x-ui.text>
                                        </x-ui.empty.contents>
                                    </x-ui.empty>
                                </td>
                            </tr>
            ...
            <div class="border-t border-black/10 dark:border-white/10 px-4 pb-3 flex justify-center">
                {{-- Own page size + paginator so the sales table above is unaffected. --}}
                <x-ui.pagination
                                ...
                                <template x-for="item in selectedTx.sale_items" :key="item.id">
                                    <tr class="hover:bg-neutral-50 dark:hover:bg-white/5">
                                        <td class="px-4 py-3">
                                            ...
                                            <p class="text-[10px] text-neutral-500" x-text="item.product?.generic_name || ''"></p>
                                        </td>
