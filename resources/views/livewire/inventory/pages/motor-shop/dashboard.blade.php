<div class="max-w-7xl mx-auto space-y-6">
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <x-ui.button href="{{ route('inventory.motor-shop.stocks') }}" icon="arrow-right">
            Add Stock
        </x-ui.button>
    </div>
        ...
        <div class="lg:col-span-1">
            <x-ui.card hoverless size="full" class="min-h-100!">
                ...
                <div class="space-y-4 overflow-y-scroll h-80 pr-2">
                    @forelse($this->recentlyReceivedBatches as $batch)
                    ...
                    @empty
                        <div class="text-center py-8">
                            ...
                            <p class="text-sm text-green-500">No received stock yet.</p>
                        </div>
                    @endforelse
                </div>
            </x-ui.card>
                    ...
                    <table class="w-full text-left">
                        ...
                        <tbody class="divide-y divide-black/10 dark:divide-white/10 bg-neutral-50 dark:bg-[#060A23]">
                            ...
                            <tr class="hover:bg-indigo-50/50 dark:hover:bg-indigo-900/20 transition-colors group">
                                ...
                                <td class="px-6 py-4">
                                    <div class="font-medium text-black dark:text-white">{{ $item->product->baseUnit->name ?? 'N/A' }}</div>
                                    <div class="text-xs text-neutral-500">{{ $item->product->baseUnit->abbreviation ?? '' }}</div>
                                </td>
    ...
    @include('livewire.inventory.pages.shared.stock-movements-ledger', [
        ...
        'stockMovementTitle' => 'Stock Movements',
        'stockMovementDescription' => 'Inventory transaction ledger for motor parts stock IN and OUT.',
        'stockMovementEmptyDescription' => 'Motor Shop inventory transactions will appear here once stock moves in or out.',
        'showStockMovementDateFilters' => true,
    ])
                        ...
                        <tbody class="divide-y divide-black/10 dark:divide-white/10 bg-neutral-50 dark:bg-[#060A23]">
                            @forelse ($this->lowStockProducts as $product)
                                ...
                                <tr class="hover:bg-white/5 transition-colors group">
                                    ...
                                    <td class="px-6 py-4 font-mono text-neutral-400">
                                        <div class="font-medium text-black dark:text-white">{{ $product->baseUnit->name ?? 'N/A' }}</div>
                                        <div class="text-xs text-neutral-500">{{ $product->baseUnit->abbreviation ?? '' }}</div>
                                    </td>
