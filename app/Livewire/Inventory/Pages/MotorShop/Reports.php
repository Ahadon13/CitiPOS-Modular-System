...
#[Layout('components.layouts.motor-shop', ['title' => 'Financial & Performance Reports', 'inventory' => true])]
class Reports extends Component
{
    ...
    protected function motorShopCategoryName(): string
    {
        return CategoryType::MotorShop->value;
    }
    ...
    public function financials(): array
    {
        ...
        $salesData = SaleItem::join('sales', 'sale_items.sale_id', '=', 'sales.id')
            ...
            ->where('product_categories.name', $this->motorShopCategoryName())
            ->select(
    ...
    public function inventorySnapshot(): array
    {
        $totalStockValue = InventoryBatch::where('branch_id', $this->currentBranchId)
            ...
            ->whereHas('product.productCategory', fn ($query) => $query->where('name', $this->motorShopCategoryName()))
    ...
    {
        $fileName = 'motor-shop-stock-movements-report-' . now()->format('Y-m-d_H-i-s') . '.xlsx';
