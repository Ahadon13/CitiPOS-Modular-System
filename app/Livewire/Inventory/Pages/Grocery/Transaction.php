...
#[Layout('components.layouts.grocery', ['title' => 'Inventory Sales', 'inventory' => true])]
class Transaction extends Component
{
    ...
    public function baseQuery(): Builder|Sale
    {
        ...
        $query->whereHas('saleItems.product.productCategory', fn (Builder $q) => $q->where('name', CategoryType::Grocery->value));
    ...
    public function topDemandProducts()
    {
        ...
        return \App\Models\SaleItem::selectRaw('product_id, SUM(quantity) as total_sold, SUM(subtotal) as total_revenue')
            ...
            ->whereHas('product.productCategory', fn (Builder $query) => $query->where('name', CategoryType::Grocery->value))
    ...
    public function processExport()
    {
        ...
        try {
            if ($this->exportTarget === 'transactions') {
                $fileName = 'Grocery_Sales_Ledger_' . $startDate . '_to_' . $endDate . '.xlsx';
                return Excel::download(
                    new TransactionsExport(
                        ...
                        search: $this->search ?? '',
                        targetCategories: [CategoryType::Grocery->value]
                    ),
                    $fileName
                );
            }
            ...
            if ($this->exportTarget === 'sales-report') {
                $fileName = 'Grocery_Sales_Report_' . $startDate . '_to_' . $endDate . '.xlsx';
    ...
    public function downloadDailyReport()
    {
        ...
        try {
            ...
            $fileName = 'Grocery_Daily_Report_' . $parsedDate->format('Y_m_d') . '.xlsx';
