...
#[Layout('components.layouts.grocery', ['title' => 'Inventory Stocks', 'inventory' => true])]
final class Stocks extends Component
{
    ...
    public function totalProductsCount(): int
    {
        ...
        $this->applyGroceryScope($query, 'productCategory');
        return $query->count();
    }
    ...
    {
        $query = Product::query()->with('baseUnit')->where('branch_id', $this->currentBranchId)->orderBy('brand_name');
        $this->applyGroceryScope($query, 'productCategory');

        return $query->get()->map(function ($product) {
            return [
                'value' => $product->id,
                'label' => $product->brand_name . ' (' . ($product->baseUnit->abbreviation ?? 'pcs') . ')',
            ];
        })->toArray();
    }
    ...
    public function loadEditBatch(int $id): void
    {
        ...
        $this->dispatch('open-edit-batch-modal',
            productName: $batch->product->brand_name
        );
    }
    ...
    {
        try {
            $fileName = 'Grocery_Stocks_' . now()->format('Y_m_d_His') . '.xlsx';
